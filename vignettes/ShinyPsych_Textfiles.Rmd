...
**dateInput**. Date input. Opens a little date panel when clicked, from which the date can be selected. Note that dateInput can lead to some strange interactions that cause the numberSlider, if used, to be displayed incorrectly.
```{r, echo=FALSE}
temp.df <- tags.df[10,]
temp.df$id <- "datInput"
temp.df$text <- "This is an example of Date input: dateInput"
temp.df$type <- "dateInput"
temp.df[, 5:10] <- NA
knitr::kable(temp.df, row.names = FALSE)
```
