...
```{r, eval=FALSE}
devtools::install_github("ndphillips/ShinyPsych")
library("ShinyPsych")
```

To see some examples working, you can either look at our apps by clicking on the links in the tutorial sections, or make use of ShinyPsych's `callApp()`function to which you can give the name of the app you want to see and whether it should be run or if the code should be shown in a new editor tab (note that showing the app code for some reason doesn't work properly on RStudio for some reason, so you'd have to use that function from the R interface).

```{r, eval=FALSE}
library("ShinyPsych")

# show survey app code (not working in RStudio - at least not on windows)
callApp("Survey", "show")
...
[Here's a tutorial for the DfD Task](ShinyPsych_DfD.html "Decisions from Description").
...
[Here's a tutorial for the DfE Task](ShinyPsych_DfE.html "Decisions from Experience").
